...
const AddNewPlayList = () => {
  ...
  return (
    <div className=" bg-black/10 flex flex-col gap-4 dark text-white p-4 scroll-smooth rounded-md lg:w-96 w-full backdrop-blur-sm">
      <div className="space-y-2">
        <h4 className="font-medium leading-none font-base">
          Add custom playlist
        </h4>
        <p className="text-xs text-muted-foreground">
          Don&apos;t know how to get the link of a playlist?{" "}
          <a
            className="text-violet-400 underline-offset-4"
            href="https://mashable.com/article/spotify-share-playlist-how-to"
          >
            learn more
          </a>
        </p>
      ...
      <form onSubmit={handleSubmit}>
        <div className="flex flex-col w-full">
          ...
          />
          {error && (
            <p className="text-rose-500 text-xs font-base ml-3 mt-2">{error}</p>
          )}
          <div className="w-full flex justify-end">
            <Button
              type="submit"
              className="w-full  max-w-24 rounded-none h-8 mt-3"
              disabled={loading}
            >
              {loading ? "Adding..." : "Add"}
            </Button>
