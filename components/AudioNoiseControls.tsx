...
const AudioNoiseControls = ({
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div
        className={`relative flex lg:flex-row flex-col-reverse gap-4 ${
          hide ? "hidden" : "block"
        }`}
      >
        <Popover>
              ...
              <PopoverTrigger>
                ...
                >
                  <SlidersVertical
                    strokeWidth={2}
                    size={20}
                    className="text-white group-hover:scale-110 duration-300 min-w-5 min-h-5 "
                  />
                </span>
              </PopoverTrigger>
            </TooltipTrigger>
          ...
          <PopoverContent
            ...
            className={`bg-black/40 backdrop-blur-xl shadow-lg m-4 flex flex-col gap-2 dark rounded-lg ${animationClass}`}
          >
            {/* dynamic player */}
            {audioSource.map((audio, index) => (
