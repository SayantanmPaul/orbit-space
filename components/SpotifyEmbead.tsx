  ...
  return (
    <TooltipProvider delayDuration={100}>
      <div>
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              className={`absolute bottom-4 lg:right-36 right-4 bg-black/20 backdrop-blur-sm rounded-lg group flex w-12 h-12 items-center justify-center cursor-pointer hover:bg-black/20 ${
                hideIcon ? "hidden" : "block"
              }`}
            >
              <AiFillSpotify
                size={24}
                className="text-white group-hover:scale-110 duration-300 w-6 h-6 object-cover my-2"
              />
            </button>
          </TooltipTrigger>
        ...
        {!disabled && (
          <div
            className={`absolute bottom-20 right-4 lg:w-96 w-80 max-h-[560px] overflow-hidden rounded-xl transition-opacity duration-500 z-10 ${
              hide ? "opacity-0 h-0" : "opacity-100"
            }`}
            id="embed-iframe"
          >
            <iframe
