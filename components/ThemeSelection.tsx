...
const AvailableThemes: themeType[] = [
  ...
  },
  {
    ...
    playlistUrl:
      "https://open.spotify.com/embed/playlist/7jiQemqr9EFtd2zlzMEQ7h?utm_source=generator",
  ...
  },
  {
    ...
    playlistUrl:
      "https://open.spotify.com/embed/playlist/1UAIEpILhZMPhpamN4Cn9w?utm_source=generator",
  ...
  },
  {
    ...
    playlistUrl:
      'https://open.spotify.com/embed/playlist/3BGlRkrccFeOaNnG5tfR88?utm_source=generator"',
...
const ThemeSelectionJSX = () => {
  ...
  const setBackgroundVolumesFromTheme = useAppStore(
    (state) => state.setBackgroundVolumesFromTheme
  );
  ...
  return (
    <ScrollArea className=" flex flex-col gap-4 dark text-white p-4 scroll-smooth w-80 max-h-72 relative bg-black/30 z-[100]">
      <div className="space-y-4 ">
        <h4 className="font-medium leading-none text-sm font-base">
          Select theme
        </h4>
        <div className="flex flex-col gap-2">
          {AvailableThemes.map((theme, index) => (
            ...
            >
              ...
              <div
                className={`absolute inset-0 flex items-center justify-center border border-white/40 hover:border-yellow-300 rounded-md duration-300 overflow-hidden bg-black/30 ${
                  selectedTheme === theme.themeName && "border-yellow-300"
                }`}
              >
                <button className="p-2 text-white font-semibold font-base shadow-2xl">
                  {theme.themeName}
                </button>
              </div>
            </div>
