...
const SettingsDialog = ({ stopTimer, timerMode }: { stopTimer: () => void, timerMode: "pomodoro" | "shortBreak" | "longBreak" }) => {
    const {
        pomodoroTime,
        setPomodoroTime,
        shortBreakTime,
    ...
    const [tempSelectedTimerColor, setTempSelectedTimerColor] = useState(selectedTimerColor);
            ...
            >
                ...
                <span className="flex justify-between w-full items-center">
                    ...
                    <div className="flex gap-3 items-center">
                        {['rgb(217, 22, 86)', '#FFBD73', '#14B8A6', '#9694FF', '#F075AA'].map((color) => (
                            <span
                                key={color}
                                className="w-8 h-8 rounded-full"
                                style={{
                                    backgroundColor: color,
                                    border: tempSelectedTimerColor === color ? '2px solid white' : 'none',
                                }}
                ...
                <span className="flex justify-between w-full items-center">
                    <Label className="font-base text-slate-300 font-medium">Clock Face</Label>
                    <div className="flex gap-3 items-start ">
                        {['Oswald', 'Times New Roman', 'Lobster'].map((font) => (
                            <span
                                ...
                                style={{
                                    fontFamily: font,
                                    backgroundColor: tempClockFace === font ? 'rgba(255,255,255,0.1)' : 'transparent',
